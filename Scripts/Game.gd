...
func _ready():
	...
	hint = generate_hint(solution)
	
	columns = solution[0].size()
	rows = solution.size()
	
	# Generate NonagramTileMap tiles based on board dimensions
	for col in columns:
...
func _input(event):
	# Get the selected tile in nonagram_tile_map space
	var selected_nonagram_tile_map_coordinates = nonagram_tile_map.world_to_map(nonagram_tile_map.get_local_mouse_position())
	
	# Check for Left Mouse Click
	if event is InputEventMouseButton and event.pressed and is_in_board(selected_nonagram_tile_map_coordinates):
	#		print("Mouse Click at: ", selected_nonagram_tile_map_coordinates)
	#		print(is_in_board(selected_nonagram_tile_map_coordinates))
		
		# If there is a left or right mouse click in board, process it
		if event.button_index == BUTTON_LEFT or event.button_index == BUTTON_RIGHT:
			set_tile(selected_nonagram_tile_map_coordinates, event.button_index)
...
func set_tile(nonagram_tile_map_coords, button_index):
	...
	if button_index == BUTTON_LEFT:
		...
		if solution_tile_map.get_cellv(nonagram_tile_map_coords) == 2 and not nonagram_tile_map.get_cellv(nonagram_tile_map_coords) == -1:
		...
		elif nonagram_tile_map.get_cellv(nonagram_tile_map_coords) == 2 or nonagram_tile_map.get_cellv(nonagram_tile_map_coords) == -1:
			# Left click colors in tiles unless it's already colored or solved (blank), then it resets
			# Already dark, reset it
			nonagram_tile_map.set_cellv(nonagram_tile_map_coords, 0)
		else:
			# Already not dark, make it dark
			nonagram_tile_map.set_cellv(nonagram_tile_map_coords, 2)
			
	if button_index == BUTTON_RIGHT:
		# Compare this action with solution tile map to see if it's a correct one
		if solution_tile_map.get_cellv(nonagram_tile_map_coords) == 1 and not nonagram_tile_map.get_cellv(nonagram_tile_map_coords) == -1:
			# If it is correct, then the tile is removed to reveal the tilemaps underneath it
			nonagram_tile_map.set_cellv(nonagram_tile_map_coords, -1)
		elif nonagram_tile_map.get_cellv(nonagram_tile_map_coords) == 1 or nonagram_tile_map.get_cellv(nonagram_tile_map_coords) == -1:
			# Right click whites out tiles unless it's already whited out or solved, then it resets
			# Already white, reset it
			nonagram_tile_map.set_cellv(nonagram_tile_map_coords, 0)
		else:
			# Not white, make it white
			nonagram_tile_map.set_cellv(nonagram_tile_map_coords, 1)
